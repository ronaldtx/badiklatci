...
class Skkonsep extends CI_Controller {
    ...
    public function index()
    {
        if($this->session->userdata('UserName')){
            ...
            $data['breadcrumb'] = '<span class="divider">
                ...
                <li class="active">Surat Keluar Dengan Konsep</li>';
            ...
            $data['listsurat'] = $this->skkonsep_model->listall($cond, $data['page']);
            $this->load->view('surat/skkonsep/list', $data);
        }
        else{
    ...
    public function create(){
        if($this->session->userdata('UserName')){
            ...
            $data['breadcrumb'] = '<span class="divider">
                ...
                <li class="active"><a href="'.base_url().'surat/konsep">Konsep</a></li>
                <i class="icon-angle-right arrow-icon"></i>
                  </span>
            ...
            $this->load->view('surat/konsep/new', $data);
        }
        else{
            redirect(base_url(), 'location');
        }
    ...
    public function edit(){
        ...
        if($this->session->userdata('UserName')){
            ...
            $v['surat'] = $_GET['surat'];
            $v['nomor'] = $_GET['nomor'];
            $data['surat'] = $this->skkonsep_model->getonedata($v);
            $data['breadcrumb'] = '<span class="divider">
                    <i class="icon-angle-right arrow-icon"></i>
                  </span>
                <li class="active"><a href="'.base_url().'surat/skkonsep">Surat Keluar Dengan Konsep</a></li>
                <i class="icon-angle-right arrow-icon"></i>
                  </span>
            ...
            $data['jsfooter'] = '<script type="text/javascript">
                $(function() {
                    ...
                    $("#btnsetuju").click(function(){
                        $("#formkonsep").attr("action", "'.base_url().'surat/konsep/savesetuju")
                        $("#formkonsep").submit();
                    });
            ...
            $data['liststatus'] = listall('t_par_status_sm');
            if(!empty($data['surat'])){
                $this->load->view('surat/skkonsep/edit', $data);
            }
            else{
                redirect(base_url()."surat/skkonsep", 'location');
            }
        }
        else{
            redirect(base_url(), 'location');
    ...
    public function savenew(){
        // print_r($_POST);
        // exit;
        if($this->skkonsep_model->createsk($_POST)=='success'){
            redirect(base_url()."surat/skkonsep", 'location');
        }
        else{
            redirect(base_url()."surat/skkonsep/create/?a=w", 'location');
        }
    ...
    public function saveedit(){
        // print_r($_POST);
        // exit;
        if($this->peminjaman_model->editkembali($_POST)=='success'){
            redirect(base_url()."kembalibarang", 'location');
        }
        else{
            redirect(base_url()."kembalibarang/edit/".$_POST['id']."/?a=w", 'location');
        }
