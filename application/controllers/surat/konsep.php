...
class Konsep extends CI_Controller {
    ...
    public function index()
    {
        if($this->session->userdata('UserName')){
            ...
            $data['jsfooter'] = '<script type="text/javascript">
                $(function() {
                    ...
                    $("#btnexport").click(function(){
                        window.open("'.base_url().'export/ekonsep?cari='.$data["cari"].'&tgl1='.$data["tgl1"].'&tgl2='.$data["tgl2"].'");
                    });
            ...
            $data['listsurat'] = $this->konsep_model->listall($cond, $data['page']);
            $this->load->view('surat/konsep/list', $data);
        }
        else{
    ...
    public function edit(){
        ...
        if($this->session->userdata('UserName')){
            ...
            $v['surat'] = $_GET['surat'];
            $v['nomor'] = $_GET['nomor'];
            $data['surat'] = $this->konsep_model->getonedata($v);
            $data['breadcrumb'] = '<span class="divider">
                    <i class="icon-angle-right arrow-icon"></i>
                  </span>
                <li class="active"><a href="'.base_url().'surat/konsep">Konsep</a></li>
            ...
            $data['liststatus'] = listall('t_par_status_sm');
            if(!empty($data['surat'])){
                $this->load->view('surat/konsep/edit', $data);
            }
            else{
                redirect(base_url()."surat/konsep", 'location');
            }
        }
        else{
            redirect(base_url(), 'location');
    ...
    public function savenew(){
        // print_r($_POST);
        // exit;
        if($this->konsep_model->create($_POST)=='success'){
            redirect(base_url()."surat/konsep", 'location');
        }
        else{
            redirect(base_url()."surat/konsep/create/?a=w", 'location');
        }

    }
    public function savesetuju(){
        if($this->konsep_model->setujui($_POST)=='success'){
            redirect(base_url()."surat/konsep", 'location');
        }
        else{
            redirect(base_url()."surat/konsep/create/?a=w", 'location');
    ...
    public function saveedit(){
        // print_r($_POST);
        // exit;
        if($this->konsep_model->edit($_POST)=='success'){
            redirect(base_url()."surat/konsep", 'location');
        }
        else{
            redirect(base_url()."surat/konsep/edit/".$_POST['id']."/?a=w", 'location');
        }
