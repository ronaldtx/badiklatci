<?php
class Konsep_model extends CI_Model {
    var $kode_kondisi = '';
    var $nama_kondisi = '';
    var $status_active  = 1;
    ...
    */
    ...
    {   
        ...
        $sql ="insert into t_trn_sk 
                (nomor , tahun , no_unitorg , kd_jenis_sd , 
                  ...
                  alamat_2, mengetahui, kepada, flag_sk, tgl_posting, user_posting)
    ...
    public function listall($cond, $a=0, $b=LIMITPAGING)
    {
        $sql = "SELECT * , (SELECT count(*) FROM t_trn_sk
                WHERE no_unitorg='".$this->session->userdata('UnitOrg')."' AND flag_sk = 1
                ".$cond.") jmldata
                FROM t_trn_sk 
                WHERE no_unitorg='".$this->session->userdata('UnitOrg')."' AND flag_sk = 1
                ".$cond."
