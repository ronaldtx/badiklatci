...
<form id="myform" class="form-horizontal" method="post" action="<?php echo base_url(); ?>user/saveedit">
    <?php echo forminput('text', 'userid', 'Userid', $user->userid); ?>
        	...
        	<select class="chzn-select" id="kd_level" name="kd_level" data-placeholder="Pilih Level...">
                ...
                <?php 
                    foreach ($listlevel as $d) {
                        echo '<option value="'.$d->kd_level.'"';
                        if($d->kd_level== $user->kd_level)
                            echo ' selected';
                        echo '>'.$d->uraian_level.'</option>';
                    }
                 ?>
            </select>
        </div>
        	...
        	<select class="chzn-select" id="unitorg" name="unitorg" data-placeholder="Pilih Unit Org...">
                ...
                <?php 
                    foreach ($listunit as $d) {
                        echo '<option value="'.$d->unitorg.'"';
                        if($d->kd_unit== $user->kd_unit)
                            echo ' selected';
